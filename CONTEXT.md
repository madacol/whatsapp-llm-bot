...
- **OutboundEvent**: Legacy delivery-shape vocabulary for the old unified outbound stream. Do not use it as a canonical internal interface or preserve it as an external compatibility layer during the Run Event/App Message migration.
- **Raw provider payload**: Diagnostic material from ACP or another provider protocol. It is input to the Agent Run Activity layer and may be retained only in provider diagnostics or raw-event logs; it must not be part of canonical Run Events, App Messages, or Presentation inputs.
...
- Raw provider payloads must die inside the Agent Run Activity layer. If any downstream module needs a fact from ACP or another provider protocol, the Agent Run Activity layer must expose that fact through canonical Run Event fields.
...
- OutboundEvent should be removed from internal seams and external event APIs as a breaking architecture migration. HTTP event polling/streaming, persisted outbound queues, replay, and transport renderers should move to Run Event and App Message shapes directly rather than keeping an OutboundEvent compatibility layer.
...
- Presentation has two semantically distinct input seams: Run Events for anything caused by an agent run, and App Messages for app-originated output not caused by an agent run. Raw provider data must not be a Presentation input or hidden dependency.
...
- Snapshot file changes detected as run reconciliation are Run Events. They are app-observed rather than provider-generated, but they are still caused by the agent run and belong behind the Agent Run Activity layer's normalization and deduplication policy.

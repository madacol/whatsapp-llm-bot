...
- Agent Runtime should target ACP only for now. Do not introduce speculative protocol-neutral runtime abstractions until there is a real non-ACP adapter. ACP still remains contained before the Run Event layer and must not leak raw provider payloads into Presentation.

...
- Agent Runtime should target ACP only for now. Do not introduce speculative protocol-neutral runtime abstractions until there is a real non-ACP adapter. ACP still remains contained before the Run Event layer, and raw ACP/provider payloads must not cross the Run Event seam.

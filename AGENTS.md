...
- For bugs or features that cross subsystem seams, prefer starting with the broadest useful regression test: a cross-seam integration test, or vertical-slice test, that follows the behavior through the real orchestration path where a user would notice the failure. Add narrower seam/unit tests afterwards only when they clarify edge cases, document a contract, or make failures easier to diagnose. Do not replace a useful broad test with two seam-only tests when the bug can live in the wiring between them.

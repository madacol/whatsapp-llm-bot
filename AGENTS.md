...
- Maintain `tasks/todo.md` as a brief kanban index. For any todo with useful context, examples, decisions, repro notes, blockers, or acceptance details, link a dedicated detail file under `tasks/` from the todo line and keep those details there instead of relying on chat history. Detail files must identify the concrete subject up front: the artifact, behavior, symptom, decision, or question the task is about. If the subject is ambiguous, say so explicitly and list the plausible candidates and evidence instead of using vague placeholders. When a task is completed, remove it from `tasks/todo.md`, move its detail file to `tasks/done/` if it has one, and add a concise entry to `tasks/done/done.md`; do not delete completed task details.

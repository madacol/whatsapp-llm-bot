...
- Maintain `tasks/todo.md` as a brief kanban index. For any todo with useful context, examples, decisions, repro notes, blockers, or acceptance details, link a dedicated detail file under `tasks/` from the todo line and keep those details there instead of relying on chat history. Detail files must name the inferred subject up front in precise, agent-chosen language: the artifact, behavior, symptom, decision, or question the task is actually about. Treat the user's wording as evidence, not authoritative terminology; if the user says "it", uses a vague label, or appears to name the wrong layer, infer the intended subject from nearby context and record the evidence. If multiple plausible subjects remain, list the candidates and what would disambiguate them before changing behavior. When a task is completed, remove it from `tasks/todo.md`, move its detail file to `tasks/done/` if it has one, and add a concise entry to `tasks/done/done.md`; do not delete completed task details.

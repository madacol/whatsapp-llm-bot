...
- Default to conversation over code. When the user asks a question, raises a concern, or discusses an approach — respond conversationally. Only implement if the change is trivial or the user signals you to go ahead. Do NOT jump to writing code unless the user explicitly asks for it.
- Research before guessing. If you don't know how an API, protocol, or library works, look it up (read source, search the web) and give a definitive answer. Never say "likely" or "probably" when you could say "I checked and here's how it works."

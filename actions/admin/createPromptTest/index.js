...
/**
 * Check an assertion against an LLM response.
 ...
 */
async function checkAssertion(assertion, response, callLlm) {
    ...
    case "tool_call": {
      ...
      return {
        ...
        message: found
          ...
          : `Expected tool call to ${assertion.tool_name}, got: ${response.toolCalls?.map((tc) => tc.name).join(", ") || "none"}`,
      };
    }
    case "no_tool_call": {
      const found = response.toolCalls?.some(

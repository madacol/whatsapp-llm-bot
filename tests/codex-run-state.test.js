...
describe("normalizeCodexFileChange", () => {
  it("normalizes file-change payloads when Codex provides a diff", () => {
    const rawItem = {
      type: "file_change",
...
describe("createCodexRunState", () => {
  it("preserves old and new text when the diff comes directly from the Codex event payload", async () => {
  ...
  it("uses the diff from a parsed apply_patch command", async () => {
  ...
  it("synthesizes a diff from filesystem snapshots", async () => {

...
- Added dispatcher and agent I/O regressions for a second `Thinking...` marker after an existing thought trace.
- Updated ACP reasoning delta assembly so a fresh `Thinking...` boundary after non-placeholder reasoning completes the current trace and starts a new trace instead of appending into the old one.
- Updated agent I/O reasoning state so a new trace after a finalized `Thought` gets its own thinking message and inspect payload.
- Existing delta synthesis and snapshot dedupe behavior remains covered by dispatcher tests.

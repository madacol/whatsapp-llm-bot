...
The user shared a WhatsApp screenshot where subagent research work appears as repeated wrench-prefixed messages that start with the raw prompt text, such as "Research task only; do not edit files and do not commit". The messages look like generic tool calls and do not show a useful subagent name or human-readable identifier.

The user asked to:

- improve subagent WhatsApp presentation;
- find an existing subagent name or human-readable identifier, or make one up when needed;
- differentiate the subagent presentation from the tool name;
- consider using inline code formatting for the identifier.

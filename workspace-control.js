...
export function createWorkspaceControl({ store, workspacePresentation, workspaceRepo = createWorkspaceRepoService() }) {
   ...
   */
  async function replace(repo, context, existing, baseBranch) {
   ...
   * @returns {Promise<WorkspaceCreationResult>}
   */
  async function create(repo, context, workspaceName, baseBranch) {
    if (!workspacePresentation) {
    ...
    if (existing) {
      ...
      if (choice === "replace" && canReplace) {
        return replace(repo, context, existing, baseBranch);
      }
      if (choice === "new") {
        return {
   ...
   * @returns {Promise<string>}
   */
  async function archiveCurrent(workspace) {
   ...
   * @returns {Promise<string>}
   */
  async function archiveByName(repo, workspaceName) {
    ...
    return archiveCurrent(workspace);
  }

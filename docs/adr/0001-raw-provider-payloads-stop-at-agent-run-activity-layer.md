...
The ACP adapter speaks provider protocol. The Agent Run Activity layer interprets provider/runtime activity and emits canonical Run Events. Presentation renders canonical app events for concrete transports such as WhatsApp.
...
If any module downstream of the Agent Run Activity layer needs a provider-derived fact, the Agent Run Activity layer must expose that fact through a canonical Run Event field.

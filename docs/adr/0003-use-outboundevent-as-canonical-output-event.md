...
Agent-run activity and app-originated output can still have different event names and fields when needed. The distinction does not require separate top-level event interfaces or mandatory generic source/cause metadata.

## Decision

Use `OutboundEvent` as the canonical app-owned output event model.

Do not split the public or internal output model into separate top-level Run Event and App Message interfaces. Prefer explicit OutboundEvent names. Add source, cause, or policy fields only when a concrete consumer needs them. For example, an agent-run cancellation and a "nothing to cancel" app reply should be different OutboundEvent names/shapes, not the same generic command result.
...
- Event names should carry meaning directly; source/cause fields are optional and should be added only when they simplify real behavior.
